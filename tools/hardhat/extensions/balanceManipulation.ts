...
export class balanceManipulation {
  ...
  async toBytes32(bn: BigNumber): Promise<string> {
    return this.hre.ethers.utils.hexlify(this.hre.ethers.utils.zeroPad(bn.toHexString(), 32))
  }
  ...
  async setBalance(tokenAddress: string, beneficiary: string, amount: BigNumberish, slot?: number) {
    if (slot === undefined) {
      slot = this.slots[tokenAddress]
      if (slot === undefined) {
        throw new Error('Slot for this Token is not known. You can specify slot directly via slot parameter')
      }
    }
    const index = this.hre.ethers.utils
      .solidityKeccak256(['uint256', 'uint256'], [beneficiary, slot])
      .replace(/0x0+/, '0x')
    await this.setStorageAt(tokenAddress, index, (await this.toBytes32(BigNumber.from(amount))).toString())
  }
}
